...
def perturbe_homography(homography, perturbation_scale=0.1):
    """Perturbe a homography matrix by adding random noise to its elements.
    
    Args:
        homography (torch.Tensor): Input homography matrix of shape (B, 3, 3) or (3, 3)
        perturbation_scale (float): Scale of the perturbation to apply
        
    Returns:
        torch.Tensor: Perturbed homography matrix of same shape as input
    """
    if homography.shape == (3, 3):
        homography = homography[None]
    
    # Create random perturbation matrix
    perturbation = torch.randn_like(homography) * perturbation_scale
    
    # Add perturbation to homography
    perturbed_homography = homography + perturbation
    
    # Normalize the last element to 1
    perturbed_homography = perturbed_homography / perturbed_homography[:, 2:3, 2:3]
    
    return perturbed_homography.squeeze(0) if homography.shape == (1, 3, 3) else perturbed_homography
