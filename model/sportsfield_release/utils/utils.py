...
def get_default_canon4pts(batch_size, canon4pts_type='lower'):
    """
    Homografi için varsayılan 4 köşe noktasını oluşturur.
    Mevcut FULL_CANON4PTS_NP() ve LOWER_CANON4PTS_NP() fonksiyonlarını kullanır.
    
    Args:
        batch_size: Batch boyutu
        canon4pts_type: Nokta tipi ('lower' veya 'full')
    
    Returns:
        torch.Tensor: [batch_size, 4, 2] boyutunda tensor
    """
    if canon4pts_type == 'lower':
        canon4pts = LOWER_CANON4PTS_NP()
    elif canon4pts_type == 'full':
        canon4pts = FULL_CANON4PTS_NP()
    else:
        raise ValueError(f"Bilinmeyen canon4pts_type: {canon4pts_type}")
    
    # Numpy array'i torch tensor'a çevir
    canon4pts = torch.from_numpy(canon4pts).float()
    
    # Batch boyutuna göre tekrarla
    canon4pts = canon4pts.unsqueeze(0).repeat(batch_size, 1, 1)
    
    # CUDA kullanılıyorsa GPU'ya taşı
    if constant_var.USE_CUDA:
        canon4pts = canon4pts.cuda()
