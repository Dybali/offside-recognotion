...
def set_data_arguments(parser):
    ...
    data_arg.add_argument('--coord_conv_template', type=str2bool,
                          default=False, help='replace redundant channels tp XY grids')
...
def set_end2end_optim_options():
    ...
    parser.add_argument('--need_spectral_norm_error_model', type=str2bool,
                        default=False, help='apply spectral norm to conv layers, for error model')
...
def set_init_guesser_train_options():
    global_config = read_global_config()
    ...
    parser.add_argument('--train_dataset', type=str, default=global_config['world_cup']['train_dataset'], help='train dataset path')
    parser.add_argument('--val_dataset', type=str, default=global_config['world_cup']['val_dataset'], help='val dataset path')
    parser.add_argument('--test_dataset_path', type=str, default=global_config['world_cup']['test_dataset'], help='test dataset path')
    parser.add_argument('--guess_model', type=str, default='init_guess', help='type of initial guess network')
    parser.add_argument('--need_spectral_norm_upstream', type=str2bool, default=False,
                        help='apply spectral norm to conv layers, for upstream initial guesser')
    parser.add_argument('--group_norm_upstream', type=int, default=0,
                        help='0 is batch norm, otherwise means number of groups')
    parser.add_argument('--warp_dim', type=int, default=8,
                        help='dimension of warp parameters')
    parser.add_argument('--imagenet_pretrain', type=str2bool, default=False,
                        help='use imagenet pretrained weights')

    opt = parser.parse_args()
    opt.command = ' '.join(sys.argv)
    opt.training = True
    opt.template_path = global_config['world_cup']['template_path']
    opt.test_dataset_path = opt.test_dataset_path
    return opt


def set_loss_surface_options():
    global_config = read_global_config()
    ...
    parser.add_argument('--error_model', choices=['loss_surface'], default='loss_surface',help='type of network')
    parser.add_argument('--guess_model', choices=['init_guess'], default='init_guess', help='type of initial guess network')
    parser.add_argument('--train_epochs', type=int, default=20, help='number of epochs for training')
    parser.add_argument('--prevent_neg', choices=['sigmoid'],
                        type=str, default='sigmoid', help='how to prevent negative error estimation')
    parser.add_argument('--imagenet_pretrain', type=str2bool, default=False,
                        help='use imagenet pretrained weights')
    ...
    parser.add_argument('--optim_type', default='adam',
                        choices=['adam', 'sgd'], help='gradient descent optimizer type')
    parser.add_argument('--directh_part', default='six', choices=[
        'full', 'lower', 'six'], help='optimize the lower 4 corners, or full 4 corners, or 6 corners')
    parser.add_argument('--lr', type=float,
                        default=1e-4, help='training learning rate')
    parser.add_argument('--batch_size', type=int, default=8,
                        help='batch size for training')
    parser.add_argument('--iou_space', default='part_and_whole', choices=[
        'model_whole', 'model_part', 'part_and_whole'], help='space for iou calculation')
    parser.add_argument('--error_target', choices=['iou_whole'], type=str, default='iou_whole',
                    help='error target for loss surface')
    parser.add_argument('--train_dataset', type=str, default=global_config['world_cup']['train_dataset'], help='train dataset path')
    parser.add_argument('--val_dataset', type=str, default=global_config['world_cup']['val_dataset'], help='val dataset path')
    parser.add_argument('--test_dataset', type=str, default=global_config['world_cup']['test_dataset'], help='test dataset path')
    parser.add_argument('--need_spectral_norm_error_model', type=str2bool, default=False,
                        help='apply spectral norm to conv layers, for error model')
    parser.add_argument('--group_norm_error_model', type=int, default=0,
                        help='0 is batch norm, otherwise means number of groups')

    opt = parser.parse_args()
    opt.command = ' '.join(sys.argv)
    opt.training = bool(training)
    opt.mode = 'train' if opt.training else 'test'
